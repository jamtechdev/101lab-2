...
const SellerListingDetail = ({ hideLayout = false }: { hideLayout?: boolean }) => {
  ...
  // ---- New quotation mode states ----
  const [useWholePrice, setUseWholePrice] = useState(true);
  const [useWeightPrice, setUseWeightPrice] = useState(false);

  // ---- Weight-based quotation ----
  // const WEIGHT_ITEMS = [
  //   { key: "scrap_iron", label: "Scrap Iron" },
  //   { key: "special_materials", label: "Special Materials" },
  //   { key: "waste_disposal_fee", label: "Waste Disposal Fee" },
  ...
  const refreshBidStatus = async () => {
    ...
    try {
      const response = await checkBidStatus({
        batch_id: batchId,
        buyer_id: buyerId,
      }).unwrap();
  ...
  // Normalize & refresh inspection registration
  const refreshInspectionStatus = async () => {
    ...
    try {
      const response = await checkInspectionStatus({
        batchId: batchId,
        buyerId: buyerId
      }).unwrap();
      ...
      const selectedCompany = companies.find(
        (c: any) => c.buyer_id === buyerId && c.status === "selected"
      );
  ...
  const handleUpdate = async () => {
    try {
      const res = await updateBuyerBid({
        buyer_bid_id: userBid.buyer_bid_id,
        amount: editAmount,
      }).unwrap();
  ...
  // Countdown timer — updates every second while auction has an end_date
  useEffect(() => {
  ...
  // Socket — join batch room and listen for real-time bid updates
  useEffect(() => {
    const batchId = Number(id);
    if (!batchId) return;
  ...
  // ---------------- Initialize user state on mount / user change ----------------
  useEffect(() => {
    // Only attempt if we have user or local storage id
  ...
  }, [user, id]); // intentionally exclude refresh functions from deps to avoid re-creating them

  // Load products and inspection schedule from batch response
  useEffect(() => {
    if (data?.success) {
      ...
      if (responseData.batch?.status) {
        setBatchStatus(responseData.batch.status);
      }

      if (responseData?.batch?.step) {
        setBatchStep(responseData?.batch?.step)
      }

      if (responseData.products) {
        setProducts(responseData.products.map((p: any) => mapApiProductToUI(p)));
      }
      ...
      // Extract inspection schedule (handle typo "insepction" in API)
  ...
  const openDialogFor = (product: any, type: "bid" | "inspect" | "message" | "bidding", mode?: "place_bid" | "make_offer" | "buy_now") => {
    ...
    if (type === "bid" || type === "bidding") {
      // Prefill form values if we have user info
  ...
  const handleJoinInspection = async () => {
    ...
    if (!selectedInspectionDate) {
      toast.error(t("listingDetail.pleaseSelectDate") || "Please select a date");
      return;
    }
    if (!selectedInspectionSlot) {
      toast.error(t("listingDetail.pleaseSelectSlot") || "Please select a time slot");
      return;
    }
    if (!companyName.trim()) {
      toast.error(t("listingDetail.pleaseEnterCompanyName") || "Please enter your company name");
      return;
    }

    const buyerId = user?.id || Number(localStorage.getItem("userId"));
    if (!buyerId) {
      toast.error("Please login to register for inspection");
      return;
    }
    ...
    const payload: any = {
      ...
      selected: editDialogData?.selected || false, // include existing selected/skipped if editing
    ...
    try {
      ...
      if (editDialogData?.registration_id) {
        ...
        result = await updateCompany(payload).unwrap(); // call your edit API
      } else {
        // NEW registration
      ...
      if (result?.success) {
        ...
        // Refresh inspection status after successful registration
  ...
  const getAvailableSlots = () => {
    ...
    const scheduleItem = inspectionSchedule.schedule.find(
  ...
  const handleSendMessage = () => {
    if (!message.trim()) {
      toast.error(t("listingDetail.pleaseEnterMessage"));
      return;
    }
  ...
  // Keep for any legacy references elsewhere
  ...
  // Fetch all bids for this batch — poll every 10s while auction is live
  ...
  const handlePlaceBid = async () => {
    ...
    // if (!documentFile) {
    //   toast.error(t(`toastError.document`));
    //   return;
    // }

    // ---- Validation ----
    const hasWholeBid =
      useWholePrice && bidAmount && Number(bidAmount) > 0;

    const hasWeightBid =
      useWeightPrice &&
      Object.values(weightPrices).some((v) => Number(v) > 0);
    ...
    // quotation_types (array)
    quotation_types.forEach((type) =>
      formData.append("quotation_types[]", type)
    );

    // ---- Whole item price ----
    if (hasWholeBid) {
      formData.append("amount", String(Number(bidAmount)));
    }

    // ---- Weight-based quotations (JSON-safe) ----
    if (hasWeightBid) {
      formData.append(
        "weight_quotations",
        JSON.stringify({
    ...
    // ---- Document file (NEW) ----
    if (documentFile) {
      formData.append("document_image", documentFile); // key name as required by backend
    }
    ...
    try {
      ...
      const errorMessage =
        error?.data?.message || error?.message || "Failed to place bid";
      toast.error(errorMessage);
    }
  };
  ...
  const handleOpenEditDialog = (bid) => {
    ...
    //   setUseWholePrice(bid.quotation_types?.includes("whole_item") && allowWholePrice);
    // setUseWeightPrice(bid.quotation_types?.includes("weight_based") && allowWeightPrice);
                    ...
                    {bidDetail && isLiveBidding && batchStep > 4 && (
                      <div className="space-y-3">
                        ...
                        <div className="flex items-center justify-between">
                          <Badge className="bg-destructive/10 text-destructive border-destructive/20 font-semibold text-xs px-2 py-0.5">
                            {t("buyer.endingSoon") || "Ending soon!"}
                          </Badge>
                          <div className="flex items-center gap-0.5 text-sm font-bold text-destructive tabular-nums">
                        ...
                        {/* Current Bid */}
                        <div>
                          <p className="text-xs text-muted-foreground">{t("buyer.currentBid") || "Current Bid"}:</p>
                          <p className="text-2xl font-bold text-foreground">
                            {currentBid > 0
                              ? `${Number(currentBid).toLocaleString()} ${bidDetail?.currency || ""}`
                              : t("buyer.noBidsYet") || "No bids yet"}
                          </p>
                        </div>
                      </div>
                    )}
                            ...
                            return (
                              <div
                                ...
                                className={`flex items-center justify-between px-3 py-2 text-xs ${
                                  isMyBid ? "bg-primary/10 border-l-2 border-l-primary" : "bg-background"
                                }`}
                              >
                                <div className="flex items-center gap-2 min-w-0">
                                  <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold flex-shrink-0 ${
                                    isMyBid ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"
                                  }`}>
                                    {bidderName[0]?.toUpperCase() || "?"}
                                  </div>
                                  <div className="min-w-0">
                                    <p className="font-semibold text-foreground flex items-center gap-1">
                                      {isMyBid ? <span className="text-primary">{t("buyer.you") || "You"}</span> : bidderName}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-8">
                <div className="lg:col-span-2 space-y-6">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 border border-border rounded bg-muted/20">
                    ...
                    <div>
                      <p className="text-[10px] text-muted-foreground uppercase tracking-wide mb-0.5">{t("listingDetail.condition")}</p>
                      <p className="text-sm font-semibold text-foreground">
                        {parsePhpArray(product.condition).map((cond) => t(`upload.conditionOptions.${cond}`)).join(", ")}
                      </p>
                    </div>
                    <div>
        ...
        <DialogContent className="max-h-[90vh] overflow-hidden">
          ...
          <div className="overflow-y-auto max-h-[calc(90vh-80px)] pr-2">
            ...
            <div className="space-y-4 py-4">
              ...
              {useWholePrice && (
                <div className="mt-2">
                  <Label>{t("buyer.wholeItemPrice")} ({bidDetail?.currency})</Label>
                  <Input
                    type="number"
                    value={bidAmount}
                    onChange={(e) => setBidAmount(e.target.value)}
                    placeholder={t("biddingStep.enterTotalPrice")}
                  />
                  {/* Show current tax mode */}
                  <div className="flex items-center gap-2 mt-1">
                    <input type="radio" checked readOnly />
                    <span className="text-sm text-muted-foreground">
                      {isTaxInclusive ? t("biddingStep.inclusiveTax") : t("biddingStep.exclusiveTax")}
                    </span>
                  </div>
                </div>
              )}
        ...
        <DialogContent className="max-w-sm text-center">
          <div className="flex flex-col items-center gap-4 py-4">
            ...
            <div className="flex flex-col w-full gap-2 pt-2">
              ...
              <Button
                variant="outline"
                onClick={() => setShowBidSuccessDialog(false)}
                className="w-full"
              >
